...
//
// Движок: MiMo V2.5 Pro (Xiaomi) через шлюз DotPoin.
// OpenAI-совместимый формат, авторизация Bearer.
//
// Этот Worker эволюционировал: Яндекс → MiMo (прямой API Xiaomi) →
// MiMo через шлюз DotPoin. Системный промпт, контракт и вся логика
// кэша не меняются — меняется только блок движка (см. метку
// ⮕ ДВИЖОК ниже), адрес и способ авторизации.
//
// Контракт:
//   вход:  {
...
// Шлюз DotPoin (OpenAI-совместимый). Был прямой api.xiaomimimo.com.
// Имя модели у шлюза совпадает с прежним — mimo-v2.5-pro.
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      return jsonResponse({ status: 'ok', agent: 'analyze', code: 'mimo-v13-dotpoin', model: MIMO_MODEL }, 200, CORS);
    }
    ...
    // ═══════════════════════════════════════════════════
    // ⮕ ДВИЖОК — MiMo V2.5 Pro через шлюз DotPoin.
    // OpenAI-совместимый формат: messages с ролями, model строкой.
    // Аутентификация — Authorization: Bearer (прямой API Xiaomi
    // принимал заголовок api-key; шлюз работает по Bearer).
    // ═══════════════════════════════════════════════════
    let mimoRes;
    try {
...
//   MIMO_API_KEY          (secret) — ключ DotPoin (llms.dotpoin.com)
