// ═══════════════════════════════════════════════════════════════════════════
// ⚠ СНИМОК ОТ 12.07.2026 — воркер `unitycode-write`
//
...
//   SUPABASE_URL          (var)    https://<твой-проект>.supabase.co
...
const ALLOWED_ORIGINS = [
  ...
  // ФОРКЕР: добавь свой origin, например:
...
Тебе дают НОВЫЙ шум и список кандидатов — уже существующих шумов Сети. Решай свободно, без жёсткого критерия: с кем из кандидатов новый шум резонирует настолько, что между ними стоит провести связь? Не нужно связывать со всеми подряд — только там, где чувствуешь настоящий отклик, не формальное совпадение тем.

Если ни один кандидат не резонирует — это нормальный ответ, не выдумывай связи из вежливости.
...
async function gatherLinkCandidates(env, newNodeId) {
  ...
  try {
    const [nodesRes, connsRes] = await Promise.all([
      fetch(`${env.SUPABASE_URL}/rest/v1/nodes?id=neq.${newNodeId}&select=id,raw_noise,essence&order=created_at.desc&limit=60`, { headers }),
...
async function runLinker(env, newNode) {
  ...
  const list = candidates
    .map((c, i) => `${i + 1}. ${String(c.raw_noise || '').slice(0, 300).trim()}${c.essence ? ' (суть: «' + c.essence + '»)' : ''}`)
    .join('\n');
  const userMsg = `Новый шум: ${String(newNode.raw_noise || '').slice(0, 300).trim()}${newNode.essence ? ' (суть: «' + newNode.essence + '»)' : ''}
...
${list}`;
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(parsed.connect)) indices = parsed.connect;
    } catch (_) {
      const m = raw.match(/"connect"\s*:\s*\[([^\]]*)\]/);
      if (m) indices = m[1].split(',').map(s => parseInt(s.trim(), 10)).filter(Number.isFinite);
    }
    ...
    await logLinker(env, newNode.id, `ok | candidates=${candidates.length} | raw=${raw.slice(0,150)} | written=${written}/${indices.length}`);
  } catch (e) {
...
{"verdict":"accept"} 
или
{"verdict":"reject","reason":"тихая причина на русском, 4-10 слов, голосом Сети, без укора"}`;

// Один заход к MiMo. Возвращает {raw} при успехе или {fail} при сбое сети/http.
async function mimoOnce(env, noise) {
  try {
    const r = await fetch(MIMO_URL, {
      method: 'POST',
      headers: {
...
async function gateCheck(env, noise) {
  // До двух заходов: второй — только если первый сорвался по сети или пуст
  let res = await mimoOnce(env, noise);
  let raw = res.raw || '';
  let lastFail = res.fail || '';
  if (!raw) {
    res = await mimoOnce(env, noise);          // ретрай
    raw = res.raw || '';
  ...
  try {
    ...
    if (parsed && parsed.verdict === 'reject') {
      return { verdict: 'reject', reason: String(parsed.reason || 'сигнал не различим').slice(0, 200), trace: 'json_reject' };
    }
  ...
  if (low.includes('"verdict":"reject"') || low.includes('"verdict": "reject"')) {
    ...
    const reason = (m && m[1]) ? m[1].slice(0, 200) : 'сигнал не различим';
  ...
  async fetch(req, env, ctx) {
    ...
    try {
      if (body.action === 'node') {
        ...
        if (env.MIMO_API_KEY) {
          const gate = await gateCheck(env, t);
          ...
          if (gate.verdict === 'blocked') {
            ...
            return json({ ok: false, error: 'gate_unavailable', reason: 'Сеть не смогла услышать сигнал. Попробуй ещё раз.', gate: gateTrace }, 200, origin);
          }
        }
        // ───────────────────────────────────────────────────────────
        ...
        if (created?.id) {
          ...
          ctx.waitUntil(runLinker(env, { id: created.id, raw_noise: t, essence: row.essence }));
