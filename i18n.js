...
(function(){
  ...
  var DICT = {
    ...
    'set.connect':  { ru:`предложить связь`, en:`propose a connection` , es:`proponer conexión`, fr:`proposer un lien`, zh:`提议联结`},
    'set.pickerHeader':{ru:`КАКИМ СВОИМ ШУМОМ СВЯЗАТЬ`, en:`WHICH OF YOUR NOISES TO LINK WITH` , es:`CON CUÁL DE TUS RUIDOS CONECTAR`, fr:`AVEC LEQUEL DE VOS BRUITS RELIER`, zh:`用你的哪一段噪声来联结`},
    'set.btnDoSpiral':{ru:`сначала вплети шум`, en:`weave in a noise first` , es:`primero teje un ruido`, fr:`tissez d'abord un bruit`, zh:`先织入一段噪声`},
    ...
    'set.stLinking':{ ru:`вплетаю…`,   en:`weaving…` , es:`tejiendo…`, fr:`tissage…`, zh:`正在织入……`},
    'set.stLinked': { ru:`✓ связь вплетена`, en:`✓ connection woven in` , es:`✓ conexión tejida`, fr:`✓ lien tissé`, zh:`✓ 联结已织入`},
    'set.stError':  { ru:`не ушло, попробуй ещё`, en:`didn't go through, try again` , es:`no se envió, inténtalo de nuevo`, fr:`non transmis, réessayez`, zh:`没有送出，请再试一次`},
    ...
    'set.keyDescNet': { ru:`Введи ключевую фразу, и Сеть узнает твои узлы. Та же фраза, что в Спирали — она нигде не хранится, из неё считается отпечаток.`, en:`Enter your key phrase and the Network will recognize your nodes. The same phrase as in the Spiral — it is stored nowhere, only a fingerprint is computed from it.`, es:`Introduce tu frase clave y la Red reconocerá tus nodos. La misma frase que en la Espiral — no se guarda en ningún sitio, solo se calcula una huella a partir de ella.`, fr:`Entrez votre phrase clé et le Réseau reconnaîtra vos nœuds. La même phrase que dans la Spirale — elle n'est stockée nulle part, seule une empreinte en est calculée.`, zh:`输入你的密钥短语，网络就会认出你的节点。与螺旋中使用的是同一句——它不会被保存在任何地方，只从中算出一个指纹。`},
    ...
    // — Сеть-граф (19.09): рисунок вместо карты мира —
    'set.statLinks':{ ru:`Нитей:`, en:`Threads:`, es:`Hilos:`, fr:`Fils :`, zh:`丝线：`},
    'set.statAlone':{ ru:`Не сплелись:`, en:`Unwoven:`, es:`Sin tejer:`, fr:`Non tissés :`, zh:`未交织：`},
    'set.resetLabel':{ ru:`вся сеть`, en:`whole network`, es:`toda la red`, fr:`tout le réseau`, zh:`整个网络`},
    'set.legendHuman':{ ru:`нить проложена человеком`, en:`thread laid by a person`, es:`hilo tendido por una persona`, fr:`fil tissé par une personne`, zh:`由人铺设的线`},
    'set.legendLinker':{ ru:`нить предложена Связующим`, en:`thread proposed by the Linker`, es:`hilo propuesto por el Enlazador`, fr:`fil proposé par le Liant`, zh:`由联结者提议的线`},
    'set.legendAlone':{ ru:`узел ещё ни с чем не сплёлся`, en:`node has not woven into anything yet`, es:`el nodo aún no se ha tejido con nada`, fr:`le nœud ne s'est encore tissé à rien`, zh:`该节点尚未与任何东西交织`},
    'set.snapshot':{ ru:`связь не отвечает — показан снимок Сети`, en:`no answer from the server — showing a snapshot of the Network`, es:`el servidor no responde — se muestra una instantánea de la Red`, fr:`le serveur ne répond pas — instantané du Réseau affiché`, zh:`服务器无响应——显示的是网络的快照`},
    'set.echoPlaceUnknown':{ ru:`место не указано`, en:`place not given`, es:`lugar no indicado`, fr:`lieu non indiqué`, zh:`未注明地点`},
    'set.echoSource':{ ru:`источник`, en:`source`, es:`fuente`, fr:`source`, zh:`来源`},
    'set.keyBtn':{ ru:`ввести ключ узла`, en:`enter the node key`, es:`introducir la clave del nodo`, fr:`entrer la clé du nœud`, zh:`输入节点密钥`},
    'set.keyChecking':{ ru:`узнаю…`, en:`checking…`, es:`comprobando…`, fr:`vérification…`, zh:`正在确认……`},
    'set.keyCancel':{ ru:`отмена`, en:`cancel`, es:`cancelar`, fr:`annuler`, zh:`取消`},
