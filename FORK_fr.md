...
page contient tout le nécessaire pour lancer ton portail **sans rompre le lien avec les autres forks**.
...
- **RLS est activé** directement dans `schema.sql` — l'anonyme n'a droit qu'à la lecture.
  Il est obligatoire de l'exécuter, sinon ta base reste ouverte en écriture à tout le monde.
...
**1. Forke le dépôt** sur GitHub et active Pages (Settings → Pages → Deploy from branch → main).

**2. Monte ta propre base Supabase** (offre gratuite) :

- exécute **`schema.sql`** en entier dans le SQL Editor — il crée les cinq tables et active RLS aussitôt ;
- si tu crées ensuite des tables à la main — n'oublie pas `NOTIFY pgrst, 'reload schema';`,
  sinon l'API REST cessera d'écrire dans la nouvelle table sans rien dire ;
- dans Settings → API récupère le `Project URL`, la clé publique (`anon`/`publishable` — pour la lecture)
  et la clé **secrète** de service (pour le write-worker ; ne la mets pas dans le client).

**3. Active Turnstile** dans Cloudflare (section Turnstile) : crée un widget → tu obtiendras
la Site Key (publique) et la Secret Key.

**4. Monte les trois workers** dans Cloudflare — copie le code du dossier `workers/` :

- **générateur** (`unitycode-petlya.js`) : secret `YANDEX_API_KEY`, ton propre `YANDEX_FOLDER_ID` dans le code ;
...
- **dans chacun, ajoute ton domaine à `ALLOWED_ORIGINS`** (en haut du fichier, cherche la marque «ФОРКЕР»).

**5. Inscris tes données dans les pages** (les emplacements exacts sont signalés par un commentaire au début de chaque fichier) :
...
**6. Vérifie** dans cet ordre : ouvre le worker dans le navigateur (il doit répondre `{"status":"ok"}`) →
ouvre la Spirale, envoie un bruit → une résonance arrivera → sur la carte, crée un nœud ou un lien
(l'écriture passe par le write-worker). S'il n'y a pas de réponse et que la console affiche **CORS** —
tu as oublié d'ajouter le domaine à `ALLOWED_ORIGINS` (étape 4).

-----

## Ce que tu peux changer librement

Tout ce qui rend le portail **tien** et n'affecte pas les autres :
...
noms du menu — «Trame»/«Motif»/le mot que tu préfères, l'important est de ne pas toucher aux
fichiers eux-mêmes, voir plus bas) · l'apparence de la carte · «Matière» et le contenu du Livre ·
tes propres adresses de workers, ton domaine et tes clés.
...
ton portail **compatible avec le Réseau commun**. Si tu les casses, ton nœud tombera hors du
consensus des futurs forks et cessera de «parler la même langue» que les autres. Ce n'est pas
...
1. **Les trois entrées de menu du protocole et les noms de fichiers.** Les noms :
   `implant.html` / `petlya.html` / `set.html`. Les libellés et le style, change-les comme tu veux
...
1. **Les contrats des workers.**
   ...
   Si tu changes le format, tes agents cesseront de s'emboîter avec ceux des autres.
1. **Le schéma de la base et RLS.** Les tables `nodes` et `connections`, les noms des
   ...
   tes nœuds ne s'inscriront pas dans la carte commune.
1. **Le Liant n'est plus un agent «du futur» — il tourne.** C'est la troisième étape dans
   ...
   avec les existants et crée des liens de lui-même (`connections.created_by = 'linker'`). Si tu
   changes sa logique dans ton fork, fais-le de sorte que les liens qu'il crée restent lisibles
...
Si ce qu'il y a **à l'intérieur** du portail t'intéresse — tu ne casseras pas le mur porteur : on
vient de te montrer où il est. Et celui qui voudra le casser partira sur sa propre branche, et
